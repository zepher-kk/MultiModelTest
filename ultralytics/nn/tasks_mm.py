...
# ===== MULTIMODAL EXTENSION START - YOLOMM Extension =====
# 多模态融合模块导入 - 插入式设计，独立于其他模块
# 注意：仅导入解析器接口，具体模块由解析器动态加载
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        for idx, m in enumerate(self.model):
            ...
            if hasattr(m, 'backbone'):
                ...
                if hasattr(m, '_mm_spatial_reset') and m._mm_spatial_reset:
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if isinstance(m, (DETECT_CLASS + SEGMENT_CLASS + POSE_CLASS + OBB_CLASS + V10_DETECT_CLASS)):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            ...
            try:
                # ===== MULTIMODAL EXTENSION START - 多模态配置检测 =====
                # For multimodal configurations, use 6-channel test input during initialization
                from ultralytics.nn.mm import MultiModalRouter
                mm_router_temp = MultiModalRouter(cfg, verbose=False)
                test_ch = 6 if mm_router_temp.has_multimodal_config else ch
                # ===== MULTIMODAL EXTENSION END =====
                m.stride = torch.tensor([s / x.shape[-2] for x in _forward(torch.zeros(2, test_ch, s, s))])  # forward
                ...
                'CUDA tensor' in str(e) or 'is_cuda()' in str(e) or 'carafe_forward_impl' in str(e) or 'Pointer argument (at 0) cannot be accessed from Triton (cpu tensor?)' in str(e):
                    ...
                    m.stride = torch.tensor([s / x.shape[-2] for x in _forward(torch.zeros(2, test_ch, s, s).to(torch.device('cuda')))])  # forward
                else:
                    raise e
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    is_backbone = False
    for i, layer_config in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
            ...
            if m == 'node_mode':
                ...
                if len(args) > 0:
                    if args[0] == 'head_channel':
                        args[0] = int(d[args[0]])
            t = m
            # ===== MULTIMODAL EXTENSION START - 多模态模块检查 =====
            if isinstance(m, str) and is_multimodal_module(m):
                # 获取多模态模块类
                mm_modules = get_multimodal_modules()
                m = mm_modules[m]
            else:
        ...
        #     args = [c2, *args]
            ...
            c2 = ch[f][args[0]]
        # ===== MULTIMODAL EXTENSION START - 多模态模块处理 =====
        # 多模态融合模块处理 - 插入式设计，最小侵入性实现
        # 具体处理逻辑由独立的mm_parser模块负责
        elif is_multimodal_module(t):  # 使用模块类型字符串检查
            c2, args = process_multimodal_layer(m, f, args, ch, width, max_channels, d)
        # ===== MULTIMODAL EXTENSION END =====
        else:
            c2 = ch[f]
