...
# Extra modules conditional import
try:
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        if hasattr(self, 'mm_router') and self.mm_router is not None:
            ...
            try:
                ...
                # Fallback: Create temporary router for other model types
                config_dict = getattr(self, 'yaml', None)
                mm_router = MultiModalRouter(config_dict, verbose=profile)
                mm_routing_enabled, mm_input_sources = mm_router.setup_multimodal_routing(x, profile)
                if profile:
                    LOGGER.info("🔄 MultiModal: 使用临时router (回退模式)")
    ...
    def __init__(self, cfg="rtdetr-l.yaml", ch=3, nc=None, verbose=True):
        ...
        if MULTIMODAL_AVAILABLE:
            try:
                ...
                if verbose:
                    LOGGER.info(f"🚀 RTDETRDetectionModel: 持久化MultiModalRouter已创建")
            except Exception as e:
                if verbose:
...
def parse_model(d, ch, verbose=True, dataset_config=None):
    ...
    for i, layer_config in enumerate(d["backbone"] + d["head"]):  # from, number, module, args, [input_type]
        ...
        #           C3k2_VSS, C3k2_wConv, C3k2_FourierConv}:  # 其他C3k2变体 - 来自extra_modules
        #     c1, c2 = ch[f], args[0]
        #     if c2 != nc:
        #         c2 = make_divisible(min(c2, max_channels) * width, 8)
        ...
        #     # M/L/X尺寸的特殊处理
        #     if scale in "mlx":
        #         # C3k2_wConv和C3k2_FourierConv有特殊的参数处理
        #         if m in {C3k2_wConv, C3k2_FourierConv}:
        ...
        #           DSConv_YOLO13, wConv2d, FourierConv}:  # YOLO13系列卷积
        #     c1, c2 = ch[f], args[0]
        #     if c2 != nc:
        #         c2 = make_divisible(min(c2, max_channels) * width, 8)
        #     args = [c1, c2, *args[1:]]
